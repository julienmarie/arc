defmodule Arc.Actions.Store do
  ...
  def store(definition, {file, scope}) when is_binary(file) or is_map(file) do
    response = put(definition, {Arc.File.new(file), scope})
    response 
  end
  ...
  defp put_version(definition, version, {file, scope}) do
    resp = case Arc.Processor.process(definition, version, {file, scope}) do
      ...
      {:ok, file} ->
        ...
        resp = definition.__storage.put(definition, version, {file, scope})
        resp 
     end
